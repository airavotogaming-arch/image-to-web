      ...
      <div className={`grid items-center gap-12 lg:grid-cols-2 ${reverse ? "lg:[&>*:first-child]:order-last" : ""}`}>
        ...
        <div className="grid grid-cols-3 gap-3">
          {mod.stats.map((s) => (
            <div
              key={s.l}
              className="panel flex flex-col items-center justify-center p-6 text-center col-span-1"
            >
          ...
          {/* placeholder card with mini list */}
          <div className="panel col-span-3 p-5">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {mod.bullets.slice(0, 3).map((b) => (
                <span
                  key={b}
                  className="rounded-full border border-border bg-surface-2 px-3 py-1 text-[11px] text-foreground/80"
                >
                  {b}
                </span>
              ))}
